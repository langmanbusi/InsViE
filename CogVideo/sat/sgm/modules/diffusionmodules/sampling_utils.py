...
class NoDynamicThresholding_edit:
    def __call__(self, uu, cu, cc, scale, scale_img):
        # video_cond, text_cond: uu, cu, cc
        scale = append_dims(scale, cc.ndim) if isinstance(scale, torch.Tensor) else scale
        scale_img = append_dims(scale_img, cc.ndim) if isinstance(scale_img, torch.Tensor) else scale_img
        return uu + scale_img * (cu - uu) + scale * (cc - cu)
