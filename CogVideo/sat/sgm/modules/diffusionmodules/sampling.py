...
from .guiders import DynamicCFG, DynamicCFG_edit
...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise_edit(self, x, z, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None):
        additional_model_inputs = {}
        ...
        if isinstance(scale, torch.Tensor) == False and scale == 1:
            ...
            if isinstance(self.guider, DynamicCFG_edit):
                denoised = self.guider(
                    denoised, (1 - alpha_cumprod_sqrt**2) ** 0.5, step_index=self.num_steps - timestep, scale=scale
                )
            else:
...
class VPSDEDPMPP2MSampler(VideoDDIMSampler):
    ...
    def sampler_step(
        ...
        if z is not None:
            denoised = self.denoise_edit(
                x, z, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb
            ).to(torch.float32)
        else:
            denoised = self.denoise(
                x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb
    ...
    def __call__(self, denoiser, x, z, cond, uc=None, num_steps=None, scale=None, scale_emb=None):
        # self.sampler(denoiser, randn, z, cond, uc=uc, scale=scale, scale_emb=scale_emb)
        x, s_in, alpha_cumprod_sqrt, num_sigmas, cond, uc, timesteps = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
