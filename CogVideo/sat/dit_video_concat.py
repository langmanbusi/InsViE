...
class ImagePatchEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        is_edit=False,
        emb_ver='double_ch', # double_ch, fix_proj
    ):
        super().__init__()
        ...
        if self.is_edit:
            if self.emb_ver == 'double_ch':
                self.proj_edit = nn.Conv2d(in_channels * 2, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
                self.proj = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
            elif self.emb_ver == 'fix_proj':
                self.proj_edit = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
                self.proj = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
                self.proj_edit_merge = nn.Linear(hidden_size*2, hidden_size)
        else:
            self.proj = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        # self.show_data()
        # now is 3d patch
        images = kwargs["images"]  # (b,t,c,h,w) if self.is_edit: (b,t,c*2,h,w)
        B, T, C = images.shape[:3]
        C = C // 2
        # B, T = images.shape[:2]
        emb = images.view(-1, *images.shape[2:])
        if self.is_edit:
            if self.emb_ver == 'double_ch':
                emb = self.proj_edit(emb)  # ((b t),d,h/2,w/2)
                ...
                emb_source = emb[:, :C, :, :]
                emb_edited = emb[:, C:, :, :]
                emb_source = self.proj(emb_source)
                emb_edited = self.proj_edit(emb_edited)  # ((b t),d,h/2,w/2)
                emb_source = emb_source.view(B, T, *emb_source.shape[1:])
                emb_source = emb_source.flatten(3).transpose(2, 3)  # (b,t,n,d)
                emb_source = rearrange(emb_source, "b t n d -> b (t n) d")
                emb_edited = emb_edited.view(B, T, *emb_edited.shape[1:])
                emb_edited = emb_edited.flatten(3).transpose(2, 3)  # (b,t,n,d)
                emb_edited = rearrange(emb_edited, "b t n d -> b (t n) d")
                emb = self.proj_edit_merge(torch.cat([emb_source, emb_edited], dim=2)) # (b,t*n,d)
        else:
    ...
    def reinit(self, parent_model=None, proj_init=False):
        if self.is_edit:
            if self.emb_ver == 'double_ch':
                w = self.proj.weight.data
                self.proj_edit.weight.data.zero_()
                self.proj_edit.weight.data[:, :self.in_channels, :, :] = w
            elif self.emb_ver == 'fix_proj':
                self.proj_edit.weight.data.zero_()
                nn.init.xavier_uniform_(self.proj_edit_merge.weight)
                nn.init.zeros_(self.proj_edit_merge.bias)
        else:
            w = self.proj.weight.data
            nn.init.xavier_uniform_(w.view([w.shape[0], -1]))
        if self.is_edit:
            if self.emb_ver == 'double_ch':
                # nn.init.constant_(self.proj_edit.bias, 0)
                proj_bias = self.proj.bias.data
                self.proj_edit.bias.data = proj_bias
            # elif self.emb_ver == 'fix_proj':
        else:
    ...
    def show_data(self):
        print_rank0(self.proj_edit.weight.data[:1, 16:18, :, :])
        print_rank0(self.proj_edit.bias.data[:2])


class ImagePatchEmbeddingMixin_1(BaseMixin):
    def __init__(
        self,
        in_channels,
        hidden_size,
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        B, T, C = images.shape[:3]
        C = C // 2
        images_source = images[:, :, :C, :, :]
        images_edited = images[:, :, C:, :, :]
        emb_source = images_source.view(-1, *images_source.shape[2:])
        emb_edited = images_edited.view(-1, *images_edited.shape[2:])
        if self.is_edit:
            emb_source = self.proj(emb_source)
            emb_edited = self.proj_edit(emb_edited)  # ((b t),d,h/2,w/2)
            emb_source = emb_source.view(B, T, *emb_source.shape[1:])
