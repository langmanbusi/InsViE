...
class SATVideoDiffusionEngine(nn.Module):
    ...
    def get_input(self, batch):
        return batch[self.input_key].to(self.dtype) # TODO
...
class SATVideoDiffusionEngine_edit(nn.Module):
    def __init__(self, args, **kwargs):
        super().__init__()
        ...
        log_keys = model_config.get("log_keys", None)
        input_key = model_config.get("input_key", "mp4")
        input_key_edit = model_config.get("input_key_edit", "mp4_edit") # for edit
        network_config = model_config.get("network_config", None)
        network_wrapper = model_config.get("network_wrapper", None)
        denoiser_config = model_config.get("denoiser_config", None)
        sampler_config = model_config.get("sampler_config", None)
        ...
        self.log_keys = log_keys
        self.input_key = input_key
        self.input_key_edit = input_key_edit # for edit
        self.not_trainable_prefixes = not_trainable_prefixes
        self.en_and_decode_n_samples_a_time = en_and_decode_n_samples_a_time
        self.lr_scale = lr_scale
        self.lora_train = lora_train
        ...
        self.noised_image_all_concat = model_config.get("noised_image_all_concat", False)
        self.noised_image_dropout = model_config.get("noised_image_dropout", 0.0)
        self.video_input_dropout = model_config.get("video_input_dropout", 0.0)
        self.use_lpips = model_config.get("use_lpips", False)
        self.lpips_weight = model_config.get("lpips_weight", 0.0)
        if self.use_lpips:
    ...
    def disable_untrainable_params(self):
        ...
        for n, p in self.named_parameters():
            ...
            # for edit 
            if 'proj_edit' in n:
                print_rank0('***** The first layer for edit: ' + str(n) + ' is set to train *****')
                flag = False #TODO 只打开处理condition部分的参数
    ...
    def reinit(self, parent_model=None):
        # reload the initial params from previous trained modules
        # you can also get access to other mixins through parent_model.get_mixin().
        if 'patch_embed' in parent_model:
            self.model.diffusion_model.mixins.patch_embed.reinit(proj_init=True)
    ...
    def forward(self, x, x_edit, batch):
        if self.use_lpips:
            loss, output = self.loss_fn(self.model, self.denoiser, self.conditioner, x, batch, x_edit)
            output = output.permute(0, 2, 1, 3, 4).contiguous().to(torch.bfloat16) 
            output_image = self.decode_first_stage(output)
            output_image = output_image.permute(0, 2, 1, 3, 4).contiguous()
            output_image = output_image.reshape(-1, *output_image.shape[2:])
            x_edit = x_edit.permute(0, 2, 1, 3, 4).contiguous().to(torch.bfloat16) 
            x_edit_image = self.decode_first_stage(x_edit)
            x_edit_image = x_edit_image.permute(0, 2, 1, 3, 4).contiguous()
            x_edit_image = x_edit_image.reshape(-1, *x_edit_image.shape[2:])
            loss_lpips = self.lpips(output_image, x_edit_image).reshape(-1)
            # loss_l2_pre = (output_image - x_edit_image) ** 2
            # B = output_image.shape[0]
            # loss_l2 = torch.mean(loss_l2_pre.reshape(B, -1), 1)
            # loss_all = (1 - self.lpips_weight) * loss.mean() + self.lpips_weight * (loss_lpips.mean() + loss_l2) / 2
            loss_all = (1 - self.lpips_weight) * loss.mean() + self.lpips_weight * loss_lpips.mean() 
            loss_mean = loss_all.mean()
        else:
            loss, _ = self.loss_fn(self.model, self.denoiser, self.conditioner, x, batch, x_edit)
            loss_mean = loss.mean()
        loss_dict = {"loss": loss_mean}
        return loss_mean, loss_dict
    ...
    def add_noise_to_video(self, video):
        sigma = torch.normal(mean=-3.0, std=0.5, size=(video.shape[0],)).to(self.device)
    ...
    def shared_step(self, batch: Dict) -> Any:
        x, x_edit = self.get_input(batch) # [B, T, C, H, W]
        if self.lr_scale is not None:
            lr_x = F.interpolate(x, scale_factor=1 / self.lr_scale, mode="bilinear", align_corners=False)
            lr_x = F.interpolate(lr_x, scale_factor=self.lr_scale, mode="bilinear", align_corners=False)
            lr_z = self.encode_first_stage(lr_x, batch)
            batch["lr_input"] = lr_z_edit

            lr_x_edit = F.interpolate(x_edit, scale_factor=1 / self.lr_scale, mode="bilinear", align_corners=False)
            lr_x_edit = F.interpolate(lr_x_edit, scale_factor=self.lr_scale, mode="bilinear", align_corners=False)
            lr_z_edit = self.encode_first_stage(lr_x_edit, batch)
            batch["lr_input_edit"] = lr_z_edit

        x = x.permute(0, 2, 1, 3, 4).contiguous()

        x_edit = x_edit.permute(0, 2, 1, 3, 4).contiguous()
        ...
        x_edit = self.encode_first_stage(x_edit, batch)
        x_edit = x_edit.permute(0, 2, 1, 3, 4).contiguous()

        if random.random() < self.video_input_dropout:  # random video drop
            x = torch.zeros_like(x_edit)
        else:
            x = self.add_noise_to_video(x)
            x = self.encode_first_stage(x, batch)
            x = x.permute(0, 2, 1, 3, 4).contiguous()
        ...
        gc.collect()
        torch.cuda.empty_cache()
        loss, loss_dict = self(x, x_edit, batch) # x: torch.Size([2, 13, 16, 60, 90])
        return loss, loss_dict

    def get_input(self, batch):
        return batch[self.input_key].to(self.dtype), batch[self.input_key_edit].to(self.dtype)
    ...
    def sample(
        ...
        **kwargs,
        ...
        samples = self.sampler(denoiser, randn, z, cond, uc=uc, scale=scale, scale_emb=scale_emb)
        samples = samples.to(self.dtype)
        return samples
    ...
    def log_video(
        ...
        **kwargs,
        ...
        x, x_edit = self.get_input(batch) # batch[self.input_key].to(self.dtype), batch[self.input_key_edit].to(self.dtype)
        ...
        N = min(x.shape[0], N)
        x = x.to(self.device)[:N]
        x_edit = x_edit.to(self.device)[:N]
        if not self.latent_input:
            log["inputs"] = x.to(torch.float32)
            # log["inputs_edit"] = x_edit.to(torch.float32)
        x = x.permute(0, 2, 1, 3, 4).contiguous()
        # x = self.add_noise_to_video(x)
        z = self.encode_first_stage(x, batch)
        # x_edit = x_edit.permute(0, 2, 1, 3, 4).contiguous()
        # z_edit = self.encode_first_stage(x_edit, batch)
        # if not only_log_video_latents:
        #     log["reconstructions"] = self.decode_first_stage(z).to(torch.float32)
        #     log["reconstructions"] = log["reconstructions"].permute(0, 2, 1, 3, 4).contiguous()
        z = z.permute(0, 2, 1, 3, 4).contiguous()
        ...
        if self.noised_image_input:
            ...
            samples = self.sample(c, z, shape=z.shape[1:], uc=uc, batch_size=N, **sampling_kwargs)  # b t c h w
            samples = samples.permute(0, 2, 1, 3, 4).contiguous()
            if only_log_video_latents:
                latents = 1.0 / self.scale_factor * samples
                log["latents"] = latents
